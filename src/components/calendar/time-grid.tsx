...
export function TimeGrid({
  ...
  useEffect(() => {
    // Open the grid around the start of the working day, like Outlook
  ...
  const perDay = useMemo(
    () =>
      days.map((day) => {
        ...
        return { day, allDay, positioned: layoutDayEvents(timed, day) };
      }),
    [days, events]
  );
  ...
  const pointerToPosition = (clientX: number, clientY: number) => {
    ...
    // RTL: the first day column is the rightmost one
  ...
  const handleCreatePointerDown = (e: React.PointerEvent) => {
    if (e.button !== 0 || eventDrag) return;
  ...
  const hasAllDayContent = perDay.some(
    ({ day, allDay }) => allDay.length > 0 || (tasksByDay.get(dayKey(day))?.length ?? 0) > 0
  );
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-xl border bg-card">
      ...
      {/* All-day row: all-day/multi-day events + tasks */}
      {hasAllDayContent && (
        <div className="flex border-b bg-muted/20">
          ...
          {perDay.map(({ day, allDay }) => {
            ...
            return (
              <div
                key={dayKey(day)}
                className="flex min-h-8 flex-1 cursor-pointer flex-col gap-0.5 border-s p-1"
                onClick={() => onCreateRange(startOfDay(day), startOfDay(day), true)}
              >
                {allDay.map((occ) => (
                  <EventChip
      ...
      <div ref={scrollRef} className="max-h-[65dvh] overflow-y-auto overscroll-contain">
        <div className="flex">
          ...
          >
            ...
            {/* Event blocks */}
            {perDay.map(({ positioned }, dayIndex) =>
              positioned.map(({ occurrence, startMin, endMin, column, columns }) => {
                const isDragged =
            ...
            {eventDrag?.moved && (
              ...
              >
                ...
                <p className="tabular-nums text-muted-foreground">
                  {formatEventTime(addMinutes(startOfDay(days[0]), eventDrag.startMin))}–
                  {formatEventTime(addMinutes(startOfDay(days[0]), eventDrag.endMin))}
                </p>
              </div>
            )}
